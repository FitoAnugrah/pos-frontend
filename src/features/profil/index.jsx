  ...
  const handleSaveProfile = async (newData) => {
    try {
      const res = await api.put('/auth/me', {
        ...
        phone: newData.noTelepon,
      });
      setProfileData(prev => ({
        ...prev,
        namaLengkap: res.data.user.name,
        noTelepon: res.data.user.phone || '-',
      }));
      setIsEditing(false);
    } catch (err) {
    ...
    return (
      <Bahasa 
        ...
        onSave={(newLang) => {
          ...
          // Context's useEffect will handle saving to localStorage
  ...
  return (
    <div className="flex-1 w-full bg-slate-50 h-full overflow-y-auto overflow-x-hidden">
        ...
        <div className="hidden md:flex items-center justify-between px-8 lg:px-12 py-6 bg-white/80 backdrop-blur border-b border-slate-100 sticky top-0 z-20">
          <div>
            <h1 className="text-2xl font-bold text-slate-800">{t('profil.title')}</h1>
            <p className="text-sm text-slate-500">{t('profil.subtitle')}</p>
          </div>
        </div>
        ...
        <div className="md:hidden flex items-center justify-between px-5 py-4 bg-white border-b border-slate-100 sticky top-0 z-20">
            ...
            <h1 className="text-lg font-bold text-blue-700">{t('profil.title')}</h1>
        ...
        <div className="p-4 md:p-8 lg:p-12 w-full max-w-screen-xl mx-auto flex flex-col">
          ...
          <div className="mb-8 hidden md:block">
            <h2 className="text-3xl font-semibold text-slate-800">{t('profil.title')}</h2>
          </div>
               ...
               <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center relative w-full">
                 ...
                 <div className="grid grid-cols-2 gap-4 w-full">
                   <div className="flex flex-col items-center justify-center p-4 bg-slate-50 rounded-xl border border-slate-100 hover:shadow-sm transition-shadow">
                     <p className="text-[10px] md:text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">{t('profil.totalShift')}</p>
                     <p className="text-2xl md:text-3xl font-bold text-slate-800">-</p>
                   </div>
                   <div className="flex flex-col items-center justify-center p-4 bg-slate-50 rounded-xl border border-slate-100 hover:shadow-sm transition-shadow">
                     <p className="text-[10px] md:text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">{t('profil.rank')}</p>
                     <div className="flex items-center gap-1.5 text-yellow-500 drop-shadow-sm">
                       ...
                       <span className="text-2xl md:text-3xl font-bold text-slate-800">-</span>
                     </div>
                   </div>
                 </div>
            ...
            <div className="w-full md:col-span-7 lg:col-span-8 flex flex-col gap-6 md:gap-8 mt-6 md:mt-0">
              ...
              <section className="bg-white p-5 md:p-8 rounded-2xl shadow-sm border border-slate-100">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-xs md:text-sm font-bold text-slate-400 tracking-wider uppercase">{t('profil.accountInfo')}</h3>
                ...
                <div className="flex flex-col gap-4 md:gap-6">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center pb-4 md:pb-6 border-b border-slate-50 last:border-0 last:pb-0 gap-1">
                    <span className="text-xs md:text-sm text-slate-500 font-medium">{t('profil.fullName')}</span>
                    <span className="text-sm md:text-base font-bold text-slate-800">{profileData.namaLengkap}</span>
                  </div>
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center pb-4 md:pb-6 border-b border-slate-50 last:border-0 last:pb-0 gap-1">
                    <span className="text-xs md:text-sm text-slate-500 font-medium">{t('profil.email')}</span>
                  ...
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center pb-4 md:pb-6 border-b border-slate-50 last:border-0 last:pb-0 gap-1">
                    <span className="text-xs md:text-sm text-slate-500 font-medium">{t('profil.phone')}</span>
              ...
              {/* Keamanan */}
              <section className="bg-white p-5 md:p-8 rounded-2xl shadow-sm border border-slate-100">
                <h3 className="text-xs md:text-sm font-bold text-slate-400 tracking-wider uppercase mb-4 md:mb-6">{t('profil.security')}</h3>
                <div className="flex flex-col gap-2">
                  <button onClick={() => navigate('/login', { state: { view: 'sandi-baru' } })} className="flex items-center justify-between p-3 -mx-3 hover:bg-slate-50 rounded-xl transition-colors group">
                    <div className="flex items-center gap-3 md:gap-4">
                      ...
                      <span className="text-sm md:text-[15px] font-semibold text-slate-700">{t('profil.changePass')}</span>
                    </div>
                  ...
                  <button onClick={() => setIsSecurityOpen(true)} className="flex items-center justify-between p-3 -mx-3 hover:bg-slate-50 rounded-xl transition-colors group">
                    <div className="flex items-center gap-3 md:gap-4">
                      ...
                      <span className="text-sm md:text-[15px] font-semibold text-slate-700">{t('profil.setPin')}</span>
                    </div>
                    <svg className="w-4 h-4 md:w-5 md:h-5 text-slate-300 group-hover:text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" /></svg>
                  </button>
                </div>
              ...
              {/* Pengaturan Aplikasi */}
              <section className="bg-white p-5 md:p-8 rounded-2xl shadow-sm border border-slate-100">
                <h3 className="text-xs md:text-sm font-bold text-slate-400 tracking-wider uppercase mb-4 md:mb-6">{t('profil.appSettings')}</h3>
                <div className="flex flex-col gap-2">
                  <button onClick={() => setIsBahasaOpen(true)} className="flex items-center justify-between p-3 -mx-3 hover:bg-slate-50 rounded-xl transition-colors group">
                    <div className="flex items-center gap-3 md:gap-4">
                      ...
                      <span className="text-sm md:text-[15px] font-semibold text-slate-700">{t('profil.language')}</span>
                    </div>
                  ...
                  <div className="flex items-center justify-between p-3 -mx-3 hover:bg-slate-50 rounded-xl transition-colors group cursor-pointer" onClick={toggleDarkMode}>
                    <div className="flex items-center gap-3 md:gap-4">
                       <div className="text-slate-400 group-hover:text-blue-500 transition-colors">
                       ...
                       <span className="text-sm md:text-[15px] font-semibold text-slate-700">{t('profil.darkMode')}</span>
                    </div>
                    <div>
                ...
                <div className="flex flex-col gap-2">
                  <button onClick={() => setIsPusatBantuanOpen(true)} className="flex items-center justify-between p-3 -mx-3 hover:bg-slate-50 rounded-xl transition-colors group">
                    <div className="flex items-center gap-3 md:gap-4">
                      ...
                      <span className="text-sm md:text-[15px] font-semibold text-slate-700">{t('profil.helpCenter')}</span>
                    </div>
                    <svg className="w-4 h-4 md:w-5 md:h-5 text-slate-300 group-hover:text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" /></svg>
                  </button>
                  ...
                  <button onClick={() => setIsKebijakanPrivasiOpen(true)} className="flex items-center justify-between p-3 -mx-3 hover:bg-slate-50 rounded-xl transition-colors group">
                    <div className="flex items-center gap-3 md:gap-4">
                      ...
                      <span className="text-sm md:text-[15px] font-semibold text-slate-700">{t('profil.privacyPolicy')}</span>
                    </div>
                    <svg className="w-4 h-4 md:w-5 md:h-5 text-slate-300 group-hover:text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" /></svg>
                  </button>
                </div>
